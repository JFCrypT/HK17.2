...
bool request_join() {
    ...
    if (g_node_state == NodeState::KEY_EXCHANGE || g_node_state == NodeState::PENDING_APPROVAL) {
        ESP_LOGW(TAG, "JOIN request ignored while node state is %s", state_name(g_node_state));
        return false;
    }
